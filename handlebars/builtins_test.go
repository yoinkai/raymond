...
var builtinsTests = []Test{
	...
	},
	{
		...
		"{{#if goodbye}}GOODBYE {{/if}}cruel {{world}}!",
		map[string]any{"goodbye": "dummy", "world": "world"},
		nil, nil, nil,
		"GOODBYE cruel world!",
	},
	{
	...
	},
	{
		...
		"{{#if goodbye}}GOODBYE {{/if}}cruel {{world}}!",
		map[string]any{"goodbye": []string{"foo"}, "world": "world"},
		nil, nil, nil,
		"GOODBYE cruel world!",
	},
	{
		...
		"{{#if goodbye}}GOODBYE {{/if}}cruel {{world}}!",
		map[string]any{"goodbye": []string{}, "world": "world"},
		nil, nil, nil,
		"cruel world!",
	},
	{
		...
		"{{#if goodbye}}GOODBYE {{/if}}cruel {{world}}!",
		map[string]any{"goodbye": 0, "world": "world"},
		nil, nil, nil,
		"cruel world!",
	},
	{
	...
	},
	{
		...
		map[string]any{"person": map[string]string{"first": "Alan", "last": "Johnson"}},
		nil, nil, nil,
		"Alan Johnson",
	},
	{
	...
	},
	{
		...
		"{{#each .}}{{.}}{{/each}}",
		map[string]any{"goodbyes": "cruel", "world": "world"},
		nil, nil, nil,
	...
	// NOTE: we test with a map instead of an object
	{
		...
		map[string]any{"goodbyes": map[any]map[string]string{"<b>#1</b>": {"text": "goodbye"}, 2: {"text": "GOODBYE"}}, "world": "world"},
		nil, nil, nil,
	...
	},
	{
		...
		map[string]any{"goodbyes": []map[string]string{{"text": "goodbye"}, {"text": "Goodbye"}}, "world": "world"},
		nil, nil, nil,
	...
	// @note: That test differs from JS impl because maps and structs are not ordered in go
	{
		...
		"{{#each goodbyes}}{{@index}}. {{text}}! {{/each}}cruel {{world}}!",
		map[string]any{"goodbyes": map[string]map[string]string{"a": {"text": "goodbye"}, "b": {"text": "Goodbye"}}, "world": "world"},
		nil, nil, nil,
	...
	// @note: That test differs from JS impl because maps and structs are not ordered in go
	{
		...
		map[string]any{"goodbyes": map[string]map[string]string{"foo": {"text": "goodbye"}, "bar": {"text": "Goodbye"}}, "world": "world"},
		nil, nil, nil,
...

	{
		...
		"{{#each goodbyes}}{{text}}! {{/each}}cruel {{world}}!",
		map[string]any{"goodbyes": func() []map[string]string {
	...
	},
	{
		...
		map[string]any{"exclaim": "!"},
		map[string]any{"detectDataInsideEach": detectDataHelper},
		nil,
		"a!b!c!",
	},
	...
	// @note Test added
	{
		...
		map[string]any{"goodbyes": []int{0, 1}, "data": []string{"foo", "bar"}},
		nil, nil, nil,
		"foobar",
	},
	{
		...
		map[string]any{"goodbyes": []string{"foo", "bar"}, "data": map[string]string{"foo": "baz", "bar": "bat"}},
		nil, nil, nil,
		"bazbat",
	},
	{
		...
		"{{#each goodbyes}}{{lookup ../data .}}{{/each}}",
		map[string]any{"goodbyes": []string{"Foo", "Bar"}, "data": struct {
			Foo string
			Bar string
		}{"baz", "bat"}},
		nil, nil, nil,
